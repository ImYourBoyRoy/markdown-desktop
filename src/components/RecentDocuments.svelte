      ...
      <div class="recent-list" aria-label="Recent Markdown files">
        {#each paths as path}
          <div class="recent-item">
            ...
            <button class="recent-remove icon-button" type="button" aria-label={`Remove ${recentDocumentName(path)} from recent files`} title="Remove from recent" onclick={() => onRemove(path)}>×</button>
          </div>
        {/each}
      </div>
    {:else}
